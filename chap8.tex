...
\vspace{10pt}
   ...
   \begin{enumerate}[1]
      ...
      for these two. 
         ...
         \item There can be 3 distinct elements from S so that 
         $t_1 t_2$ has the form $(a_x a_z)(a_x a_y)$. (There are several 
         variations which are all the same permutation and can be
         rewritten in this form.) Again, from the hint we can rewrite
         this as $(a_x a_y a_z)$. So rewrite $f=(a_x a_y a_z) t_3 
         \ldots t_{2n}$.

         \item There are 2 distinct elements from S so that 
         $t_1 t_2$ has the form $(a_x a_y)(a_x a_y)$. This can
         be rewritten as $\epsilon$. So rewrite $f=t_3 \ldots t_{2n}$.

         \end{enumerate}

         In each case after rewriting we have replaced $t_1 t_2$ with
         cycles from $U$ or removed them completely. Repeat this process
         for every pair of transpotitions $t_{2m+1} t_{2m+2}$. 

         When you are done you will have only elements from $U$ and
         your are done.
         Or, it's possible you have $\epsilon$ when you are done. In
         that case write
         \begin{align*}
             f &= (a_1 a_2)(a_3 a_4)(a_1 a_2)(a_3 a_4) \\
               &= (a_3 a_2 a_1)(a_1 a_3 a_4)(a_3 a_2 a_1)(a_1 a_3 a_4) 
         \end{align*}

         Or if you only have 3 elements $f=(a_1 a_2 a_3)^3$.
         Also note that this proof is impossible if the set of elements
         making up $G$ has less than 3 elements.

	 We conclude that $U$ generates $A_n$ since any arbitrary element
	 of $A_n$ can be rewritten using cycles of length 3. Therefore,
	 every element of $A_n$ can be rewritten using cycles of length 3.


        \item Prove that the set $U_1 = \{(123), (124), \ldots, (12n)\}$
        generates $A_n$. [\textsc{Hint}: $(abc)=(1ca)(1ab)$, $(1ab) = 
	(1b2)(12a)(12b)$, and $(1b2)=(12b)^2$.

	\noindent \solution Take any element $f \in A_n$ and using Part 3
	rewrite it as the product of 1 or more cycles of length 3.
	Now for each cycle in the product of $f$ make the following 
	transformation.

	If the cycle does not have the element $1$ in it then it is of the
	form $(abc)$ where $a \ne 1$, $b \ne 1$ and $c \ne 1$. Replace this
	cycle with $(1ca)(1ab)$. When we are done we'll have no cycles
