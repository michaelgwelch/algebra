...
\begin{enumerate}[A.]
   ...
   \begin{enumerate}[1)]
      ...
      2) Facts:
      \begin{enumerate}[i.]
        \item $G$ has $n$ elements. We know this because we are told
          it has order $n$.
        \item There is some element $a \in G$ such that $a^n = e$. We
          know this because we are told that $G$ has an element of
          order $n$. 
      \end{enumerate}
      So now we want to show that $G$ is cyclic (all elements are
      generated by one element). Let's assume that $G$ is not
      cyclic. Then there is one element, $b \in G$, that is not
      generated by $a$. Let's assume that element is $\epsilon$. In
      other words, let's assume that $b = \epsilon$. Well, this cannot
      be the case because we know that $a^n = \epsilon$ and we have
      assumed that $b$ is not a power of $a$. So let's assume that
      there is some other element $b \ne \epsilon$ that is not
      generated by $a$. Then let's count how many elements (at a
      minimum) would be in $G$. We have the following elements in $G$:
      $\{\epsilon, b, a, a^2, a^3, \ldots, a^{n-1}\}$. The number of
      elements (at a minimum) in $G$ is then $n+1$. (Count $n$ powers
      of $a$ and the element $b$.) But this contradicts what we are
      told that the order of $G$ is $n$. Therefore, $G$ must be
      cyclic; all elements of $G$ are generated by one
      element. $\qedhere$ 
