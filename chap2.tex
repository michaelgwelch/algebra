...
\item{\textbf{Examples of Operations}} 
  ...
  \brk \noindent \textbf{Example} $a * b = \displaystyle \frac{a + b}{ab}$,
  on the set $\Integers$.
  ...
  \item $a*b = a \ln b$, on the set $\{x \in \Reals : x > 0\}$.
    ...
    \brk \noindsol This is not an operation because it is not
    closed. A counter example is obtained by choosing $a=1$ and
    $b=0.5$: $1 * 0.5 = \ln 0.5 \approx -0.693 < 0$.  \brk


      % #3
  \item $a*b$ is a root of the equation $x^2 - a^2 b^2 = 0$, on the
    set $\Reals$.

    \brk \noindsol This is not an operation because it is not uniquely
    defined.  If we pick $a=2,b=2$ then we have $x^2 - 2^2 2^2 =
    0$. The roots of that equation are $x = \pm 4$. This means that
    $a*b$ is not uniquely defined, therefore $*$ is not an operation.
    \brk
  ...
  \item $a * b = \abs|a-b|$, on the set $\{ n \in \Integers : n \ge 0\}$.
